...
export default function ProgressGraph({ history, animKey, animDir, isBodyweight, height = 64 }) {
  ...
  const label = isBodyweight ? 'Reps Progress' : 'Weight Progress (kg)';
  const yMargin = height > 80 ? { top: 8, right: 16, left: -24, bottom: 4 } : { top: 12, right: 16, left: -28, bottom: 4 };
  const tickSize = height > 80 ? 10 : 9;
  ...
  return (
    <div className={`mb-2 rounded-xl overflow-hidden ${animDir === 'remove' ? 'new-seg-out' : 'new-seg-in'}`} style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #f5f3ff 100%)', padding: '8px 4px 4px' }}>
      ...
      <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest text-center mb-1">{label}</p>
      <ResponsiveContainer width="100%" height={height}>
        <LineChart data={data} margin={yMargin}>
          <YAxis domain={['dataMin - 1', 'dataMax + 1']} tick={{ fontSize: tickSize, fill: '#9ca3af' }} />
